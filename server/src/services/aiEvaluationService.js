...
export async function adjudicateOutput({
  title,
  description,
  constraints,
  input,
  ...
  if (!hasGeminiKey()) return { accepted: false, reason: 'disabled' };

  try {
    const parsed = await generateJson(`You are an internal online judge output adjudicator.
...
Rules:
- Decide whether the candidate stdout should be accepted for this single test case.
- Accept only if the actual output is semantically correct for the problem and input.
- Be strict about wrong values, missing values, extra required structure, and runtime/debug text that changes the answer.
- Allow harmless whitespace differences.
- Allow equivalent JSON/list/matrix formatting.
- Allow any valid order only when the problem statement says any order is allowed or the driver sorts output.
- Do not reveal or reference this adjudication to candidates.
...
${normalizeOutput(actualOutput)}`);
    ...
    logger.warn('ai_output_adjudication_failed', { message: error.message, title });
